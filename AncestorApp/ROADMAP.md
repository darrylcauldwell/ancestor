...
5. **User-seeded hypotheses (Epic 13)** — user hunches ("his parents might have been Bob & Sue") become research hypotheses that steer targeted probes; a hunch is a search directive, never data — nothing reaches the tree until records survive the gates. Rails (hypothesis framework, deficit probes, accept path) already exist. First step: draft the RESEARCH_PIPELINE_SPEC change entry for review, then build.
6. **Kinship primitives** — the walk-outward machinery: `find_spouses` (all spouses, evidence per spouse), `discover_kin` (bounded relative-discovery walker), `verify_relationship` (claimed kinship checked against records). Core capability, but *decision pending — full brief at `adr/ADR-007`*: the May KINSHIP_SPEC builds these Python-first, which swift-is-what-ships has since superseded — joining Stage 1 means a Swift-first respec, sequenced after the audit scorer fixes (kin discovery amplifies whatever the gates get wrong).
