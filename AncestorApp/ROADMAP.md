...
10. **Data-quality — muddle detection + duplicate merge** — [✓ shipped] 2026-07-16/17, surfaced by hands-on real-tree cleanup (Abraham Twyford patronymic muddle; William Henry Keyworth's phantom wives). A coherent thread that moves duplicate/muddle repair from expert manual work toward app-guided:
    ...
    - **Queued — Profile Sources & Records ledger (`PROFILE_SOURCES_LEDGER_SPEC.md`, proposed 2026-07-18).** A per-profile evidence ledger: see applied records in full without a re-run, *remove* a bad record (reverses its absorption via the existing transaction-revert), and remember the rejection so re-runs don't re-add it. Fixes two frustrations surfaced on George Herbert Brooks (records invisible on the profile → re-run to see them → re-run re-clusters worse; namesakes keep returning). Mostly wiring existing primitives (`created_by_transaction_id` revert, `absorptionPlan`, `record_rejections`). **George parked** at owner's request 2026-07-18; the @234/@539 father-son structural tangle is a *separate* edge-repair track (ledger non-goal).
