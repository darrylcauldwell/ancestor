...
10. **Data-quality — muddle detection + duplicate merge** — [✓ shipped] 2026-07-16/17, surfaced by hands-on real-tree cleanup (Abraham Twyford patronymic muddle; William Henry Keyworth's phantom wives). A coherent thread that moves duplicate/muddle repair from expert manual work toward app-guided:
    ...
    - **Profile Sources & Records ledger (`PROFILE_SOURCES_LEDGER_SPEC.md`)** — **[✓ Changes 1+2+3+4 shipped; Change 5 remains]**. Read-only ledger `03cf0d3`; **per-record removal `e17b6dd` 2026-07-21** (the delicate part the spec parked): a trash button on each ledger row reverses everything the record's apply fanned out — directionally — and remembers the rejection so research won't re-add it. NOT built on the spec's imagined record→transaction link (impossible: one apply spawns many unlinked `.manualEdit` transactions, `field_sources` has no record-id column); instead **plan-guided** — re-walks the record's `absorptionPlan` and inverts each item against current state in one audited transaction (delete rows keyed (profile, field, origin, raw) unless a sibling record corroborates; revert the column only with a `field_changes` proof this origin set it, else keep the value; recompute+delete life events; clear marriage-edge fill on exact match; delete open date/location fieldValue disputes + marriage spouseIdentity disputes with the sweep now excluding discarded evidence; `discarded` + `record_rejections`). Reversible by re-applying from research. Reviewed inline (parallel review blocked by a spend limit) — two fixes: origin-gated column revert (don't clear a value another origin set) and dispute-deletion scoped to sweep-re-derived fields; 12 tests, full suite 2874 green. **Remaining: Change 5** (muddle-detector "Review records" deep-link into the ledger). Motivating case George Herbert Brooks; the @234/@539 father-son tangle is a separate edge-repair track (ledger non-goal).
