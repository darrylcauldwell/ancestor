...
10. **Data-quality — muddle detection + duplicate merge** — [✓ shipped] 2026-07-16/17, surfaced by hands-on real-tree cleanup (Abraham Twyford patronymic muddle; William Henry Keyworth's phantom wives). A coherent thread that moves duplicate/muddle repair from expert manual work toward app-guided:
    ...
    - **Queued — IMPORT_DEDUPE Changes 4–6 (phantom-spouse cleanup, proposed 2026-07-17).** Extends the orphan-stub detector to the *single-spouse-edge* phantoms it currently skips (the Gerty / Elizabeth Carroll shape) + a plain-language guided review card so a non-expert resolves them without leads-spelunking or the raw merge tool. **Build-gated** on manual-merge verification of the Keyworth end-state — that verified state is Change 5's acceptance oracle (`a1b88be`; spec §"Phantom-spouse extension").
