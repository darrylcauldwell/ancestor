...
**Status:** Living (updated 2026-07-21). This file routes: where each phase stands and which document owns what happens next. Per-spec change lists stay authoritative on design (see `AncestorApp/README.md` for the full document index).
...
The core push is done; these are the only genuinely-open Stage-1 items.

- **SOURCE_WEIGHTING live verification** — Changes 0–5, 7, 8 shipped 2026-07-15; pending: set project Home county, FS re-auth (FS re-shipped 2026-07-21 — likely now unblockable), enter Elsie Twyford's known facts then re-run her (anchored + married-name burial hunt), one anchored run + one Kenneth-class ladder run, dispatch-log query-count comparison vs the 2026-07-14/15 campaign. — `SOURCE_WEIGHTING_SPEC.md`
- **2026-07-15 defect dossier** — triage the 6 lettered defects and carry survivors (a CWGC Cookiebot-500 + silent give-up, b stale MCP `get_run_status`, c leads lack IDs over MCP, d FS junk-persona lead gate, e FS self-narrowing pacing, f housekeeping bulk-dismiss + Ian/Kenneth import inversion); several may be stale given later FS/MCP work. — (no spec)
- **RESEARCH_PIPELINE Part II tail** — eval-harness Swift/MCP backend (§5.8.8), §5.9 incrementality refactor, §5.10 research-button collapse + auto-escalation, §5.11 hypothesis investigation, §7.5 `DeficitQueryResult` 3-state, T8a/T9/T31/T23, §5.12 five UX design passes, §14.B.2–6 MCP auto-approval Phase 2, §10.3 per-source tolerances. — `RESEARCH_PIPELINE_SPEC.md`
- **SANDWICH gate-repair campaign** — the ~14 OPEN DS findings, sequenced highest-tree-damage first (DS-01/17, then the shared name-gate ladder DS-16/05/06, then FP/FN residues, then GPS-honesty DS-19/18/21/22/23, then DS-27 dead-code); re-verify DS-18/15/12 scope vs current code before triage. — `SANDWICH_AUDIT_2026-07.md`
- **CONNECTOR deferred tail** — FT-19 (place-scoping-L), FT-21 (witness-probes-L, blocked on SourceRecord record-role), the UV-01/02/06/07/08/09 finish-interrupted-verification batch (verify before building), and the §8 T1-C1..C4 unverified critic morsels. — `CONNECTOR_AUDIT_2026-07.md`

*Everything else in Stage 1 shipped 2026-07-11..21 (connector audit, model evolution E1–E4, engine foundation C+D, conflict layer CL1–CL6, user-seeded hypotheses, evidence absorption + triage rework, muddle/merge data-quality, census-roster enrichment, detached record-review window, married-surname + name-enrichment absorption, tree-popover retirement, life-events research axes, given-name variants, leads-rework tails, RunRequestWatcher off-main, project onboarding + Getting Started) → git-only.*
