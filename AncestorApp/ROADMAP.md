...
**Status:** Living (updated 2026-07-17). This file routes: where each phase stands and which document owns what happens next. Per-spec change lists stay authoritative on design (see `AncestorApp/README.md` for the full document index).
...
| FamilySearch architecture | **PIVOT 2026-07-21 (owner): FS is NOT a data source** — the free direct sources cover UK vital-record data (with caveats: indexes-not-images, uneven volunteer coverage, E&W>Scotland). FS's unique value = **(a) hints/enrichment → leads** (Tree API, via the firewall) and **(b) document-image POINTERS → link-only citation enrichment** (Find a Grave posture — never download/store/embed; much UK imagery is Findmypast/Ancestry licence-walled so pointers are partial). **Records-read leg DELETED** (aggressive clean-slate; recover from git `9facbc1`) — removed `FamilySearchSource` + the cookie stack (`FamilySearchCookieStore`/`FamilySearchAuthView`/`FamilySearchTestProbe`) + `FamilySearchSourceTests` + the `SourceBootstrap` registration + the Settings cookie section; KEPT the OAuth foundation (`FamilySearchOAuth`) + FS citation/tier/firewall IDENTITY (string-keyed on "familysearch", plugin-independent). The enrichment integration is designed FRESH against the live Tree API (spike first), not the records-leg code. Beta AppKey received 2026-07-13; FS registered realm+callbacks (owner-confirmed 2026-07-21). **OAuth sign-in + connection-check UI SHIPPED `daafed0`** — live sign-in + Verify VERIFIED on Beta 2026-07-21 ("Connected as Darryl8160" — token accepted by apibeta/platform/users/current). **Tree-API diagnostic probe SHIPPED `9884ace`** (Settings → FS(Beta): Search tree = GET /platform/tree/search; Record hints = GET /platform/tree/persons/{id}/matches; dumps raw response, read-only). **NEXT: run the probe on Beta** to answer (a) hints read-only-vs-contribute-first, (b) viewable image ARK vs licence wall → those real responses drive the fresh enrichment spec. Developer-level = non-production/Beta-only until the compliance demo (auth+read+WRITE on Beta) promotes the app. | adr/ · FAMILYSEARCH_SOURCE_SPEC.md · GEDCOMX_CONCEPT_MAPPING.md |
...
9. **Evidence absorption + Triage leads rework** — [✓ shipped] 2026-07-16 (both specs removed post-completion; in git history). **Evidence absorption:** census/record nuggets now route to their homes — birthplace→birthLocation (county-composed, anchors the subject), occupation/residence → typed LifeEvents, birth/death corroboration from age + FindAGrave + probate, one declarative `absorptionPlan` the write path walks, and a review-time "will add" preview (`40b106b`, `b347351`, `5f79cc8`, `b215d8e`, `07c4c14`). **Triage leads rework:** search across findings+leads; identity-grouping ("N records"); Research → review → **one-click create-on-accept** (attach-to-existing via `ProposalDedup`, else create-new); **Add-as-parent** captures hard-to-find maiden names as placeholder parents; reversible Dismissed section; **blind Promote removed** (minted incomplete profiles) (`e885486`, `8adb202`, `0afc06d`, `d7dbdd0`, `755fc77`, `0fa82d1`, `de695fb`, `5c1a2cd`). Also: Triage scroll-beachball fix — leads rendered as lazy children (`512c520`); stable-`id` sort tiebreak stopping list reordering (`3e7b4f6`); four compiler warnings cleared (`2204b13`).
   - **Census-roster cross-profile enrichment (2026-07-19).** The absorption's missing "sideways" half: fill an EMPTY birth year on a relative *already linked* to a census subject (parent/sibling/spouse/child), calculated from their age in the applied census roster. Pure matching engine `AncestorKit/CensusAgeEnrichment.swift` (10 tests) — `.calculated` (CAL, ±1) date so provenance reads "derived from the census", **gap-fill only — never overwrites**, **two-way-unique name matching** (skips ambiguous "two Johns"). **Surfaced as a persistent Tasks audit + one-click, NOT a research-review discovery** — `CensusAgeBirthYearRule` (in AuditRule.swift, `.issue` category so Tasks doesn't route it out; reads `snapshot.lifeEvents` census households; 5 tests) + `AppState.setBirthYearFromCensus` + a "Set birth year ~YYYY" button in `UnifiedTasksView` mirroring the Married-Surname one-click. (An earlier build wired it into `ClusterReviewView`'s Discoveries panel — reverted: that surface only exists during an active research review and was undiscoverable; the Tasks audit sweeps all applied censuses tree-wide instead.) Motivation: great-grandfather Ernest's 1891 household left his linked parents John & Elizabeth stuck at 1/6 with no birth year despite the census stating their ages.

10. **Data-quality — muddle detection + duplicate merge** — [✓ shipped] 2026-07-16/17, surfaced by hands-on real-tree cleanup (Abraham Twyford patronymic muddle; William Henry Keyworth's phantom wives). A coherent thread that moves duplicate/muddle repair from expert manual work toward app-guided:
    - `MuddledIdentityRule` — audit detector flags a profile whose birth↔death span exceeds a threshold (two people collapsed into one); caught Abraham Twyford (`71c19a7`).
    - **Profile Merge finally wired into `CompareProfilesView`** — IMPORT_DEDUPE Change 3's `ProfileMergeEngine` execution primitive existed but was UI-unreachable; now behind a "Merge…" action with a **`MergeSafety`** guard that *blocks* merging a directly-linked parent/child (the father/son false-duplicate trap) and *warns* on dateless/divergent pairs (`c927369`); merge preserves the loser's life events + attachments so nothing is lost (`3a1c2b1`).
    - `ImpossibleParentageRule` — standing safeguard for parent-edge age/role contradictions; returned **zero** on the current tree (the Keyworth "reversed edge" hypothesis was investigated and *disproved* — edges were correct; misdiagnosis owned and the rule's doc corrected), kept as a general check (`f644754`, `4a25dba`).
    - `POSSIBLE_PEOPLE_CONTEXT_SPEC` — [✓ shipped] cluster cards carry tree context (surfaced-by origins + dates + namesake flag), by-person grouping, and a profile deep-link into the panel (`6a9e829` + thread).
    - **[✓ shipped 2026-07-18] IMPORT_DEDUPE Changes 4–6 (phantom-spouse cleanup) + option-A citation salvage.** `PhantomSpouseDetector`/`PhantomSpouseRule` extend the orphan-stub detector to the *single-spouse-edge* phantoms it skipped (the Gerty / Elizabeth Carroll shape); a plain-language guided cleanse card (Combine into the right spouse / These were separate people / Keep everything) resolves them without leads-spelunking or the raw merge tool; a v49 "reviewed" marker stops re-nag; the same two detectors run at import *and* on the on-demand scan (one detector, every trigger). **Option A** adds `salvageCitedFieldSources` to the single `performProfileMerge` choke point (card + Compare) so merges no longer drop the loser's cited records. Build-gate satisfied first: the manual Keyworth merge was run + verified via MCP (exactly Emma Gladwin + Elizabeth Wallace, no duplicate edge) as Change 5's acceptance oracle. 10 `PhantomSpouseDetectorTests`; full suite 2750 green. Also fixed: the "Compare with…" picker rendered an empty sheet (`.sheet(item:)` not `isPresented`+`if let`). Spec `IMPORT_DEDUPE_SPEC.md` (§build-notes has the AC1-vs-reality resolution).
    - **Profile Sources & Records ledger (`PROFILE_SOURCES_LEDGER_SPEC.md`)** — **[✓ COMPLETE — all changes shipped 2026-07-18/21; spec now git-only]**. Read-only ledger `03cf0d3`; **per-record removal `e17b6dd`** (the delicate part the spec parked): a trash button on each ledger row reverses everything the record's apply fanned out — directionally — and remembers the rejection so research won't re-add it. NOT built on the spec's imagined record→transaction link (impossible: one apply spawns many unlinked `.manualEdit` transactions, `field_sources` has no record-id column); instead **plan-guided** — re-walks the record's `absorptionPlan` and inverts each item against current state in one audited transaction (delete rows keyed (profile, field, origin, raw) unless a sibling record corroborates; revert the column only with a `field_changes` proof this origin set it, else keep the value; recompute+delete life events; clear marriage-edge fill on exact match; delete open date/location fieldValue disputes + marriage spouseIdentity disputes with the sweep now excluding discarded evidence; `discarded` + `record_rejections`). Reversible by re-applying from research; 12 tests. **Change 5 — muddle deep-links `60f8fa8`**: `MuddledIdentityRule` + kin `RecordAfterDeathRule` findings get a "Review records" Tasks action → opens the flagged profile's ledger expanded + scrolled (new `requestLedgerReviewProfileID` intent, `ScrollViewReader`); find→fix one click apart; rule IDs pinned by test. All reviewed inline (parallel review blocked by a spend limit). Motivating case George Herbert Brooks; the @234/@539 father-son tangle is a separate edge-repair track (ledger non-goal).
...
- **Life events feed research axes** — **[✓ shipped 2026-07-21 `3aa2c56`]** (surfaced
  2026-07-15, Elsie Twyford's research-inert Youlgreave Residence event). As designed:
  `ResearchSubject.fromProfile` derives `residenceAxes` (place + chapman + year window) and
  `burialPlace`/`burialChapmanCode` from `snapshot.lifeEvents` (sensitive excluded; place →
  county via the shared `chapmanCode(forPlaceText:)`). FS census rides the
  census-year-covering axis (county-composed); FS burial + FindAGrave prefer the burial
  place; FreeCEN/FreeREG add event counties ADDITIVELY through the FT-27 batching gate with
  umbrella expansion. Absorption-minted residence events now close their window (endDate =
  date). Hardened by a 20-agent adversarial review pre-commit (16 confirmed findings fixed,
  2 critical — batching-gate bypass, national-sweep leak); 19 pinning tests. Query-side
  given-name variants shipped the same arc (`78ab71e`). Companion nit still open: the place
  gazetteer (uk-places.json, 261 entries) has no village-level coverage — freeform fallback
  works, but village → county derivation only fires when the user appends ', Derbyshire';
  consider parish-level gazetteer data (RegionConfig parishes exist for DBY already).
