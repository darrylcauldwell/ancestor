...
**Status: Accepted 2026-07-13 (Darryl). ALL SIX CHANGES SHIPPED 2026-07-13** — CL1 producer core, CL2 standing sweep, CL3 honest reporting, CL4 witness identity, CL5 R2 ladder + death disambiguation, CL6 investigation seeding + MCP surface. **UI pass shipped 2026-07-13 (same day):** choose-one candidate card (⟨G5⟩, profile "Investigations" section), structural resolve actions (choose-parent with undo-compatible edge removal / keep-both / clear-death / discard-event / dismiss / defer), ⟨G12⟩ proposed-resolution banner on open date disputes, and witness-summary + ladder-trace display in the resolve sheet. Sole remaining affordance: the F4b one-tap "investigate as second marriage" seed (`.secondMarriage` kind exists; the dispute row + dismiss/defer actions cover the workflow meanwhile). (Drafted 2026-07-13 from the DS audit + three-design competition; detection-first won 9/9/9.) Commits will reference `#CL-Change1`…`#CL-Change6` of this spec.
