...
## Cross-run extension (2026-07-31, same day — found by owner dogfood)

**The hole:** the in-run pass sees only the batch; the negative/dedup caches often keep an earlier run's rivals OUT of the batch — so a namesake returns as an "unrivalled" fact while its competitors sit in the store (live specimen: the 1891 Ilkeston census re-promoted "Will apply" on re-research while Hayfield + Belper stayed cache-suppressed).
**The fix:** `RecordScorer.applyExclusivityAcrossStore(batch:storedFacts:)` — the same rules over batch ∪ stored facts, wired into `ResearchRunService.persist`: batch demotions are saved demoted, excluded from fact-apply, and surfaced as Triage leads; stored rivals are re-saved demoted in place (the upsert preserves `user_status` and `applied_at`). Stored-only rivalries heal too — every subsequent run incrementally cleans stale multi-fact piles, so the pre-fix history self-repairs without a dedicated sweep. 4 corpus tests incl. the self-rivalry guard (a record never competes with its own stored copy) and stored-leads-never-resurrect. Full suite 3,458 green.

## Registration-identity + display alignment (2026-07-31, third owner-dogfood round)

Live re-run after the cross-run fix exposed two more defects, both fixed same-session: (1) **the death fact demoted against its own registration TWIN** — the same GRO entry (7b/920) stored as two FreeBMD index rows. Fix: within a slot, rows group into CANDIDATES by `registrationKey` (type|vol|page|year|district; row-id fallback when no vol/page) — twins are one candidate, rivalry requires ≥2 distinct registrations, a candidate is discriminated if ANY of its rows is. (2) **The review UI showed pre-persist verdicts** ("Will apply" on a record the store had demoted). Fix: the cross-store pass moved INTO the pipeline's pre-clustering point (via the injected per-profile evidence lookup), so the displayed result and the persisted store carry identical verdicts; persist re-derives the same pass (deterministic) for stored-row demotion writes. 4 twin tests incl. the exact live shapes; full suite 3,462 all green.

## Cluster snapshot + ghost rivals (2026-07-30 dogfood, fourth round)

The fourth live re-run verified the STORE fully correct (death + corroborated marriage = only facts; every 1891 census a lead carrying its persisted `.exclusivity` reason) yet the review UI still said "Will apply" — two final defects:

1. **The cluster snapshot predated the pass.** `clusterInput` was computed from `state.scoredRecords` one statement BEFORE the round-3 pass reassigned it; `ScoredRecord` is a value type, so the clusters (which feed the review UI's pill AND its Apply path via `wouldApply`) carried stale pre-pass `.fact` copies. Clicking Apply would have written a demoted namesake to the profile. Fix: the pass runs before the snapshot is taken — the review page, the Apply path, and the store now provably share one set of verdicts.
2. **Ghost rivals (the flip-flop hole).** Demoting ALL rivals empties the slot — and only stored FACTS counted as rivals, so the next cache-suppressed lone re-fetch of any namesake would re-promote as "unrivalled", flip-flopping forever (the post-round-3 store shape made this deterministic: zero census-1891 facts + three exclusivity-demoted leads). Fix: stored leads this pass itself demoted (`isExclusivityGhost`: lead + persisted `.exclusivity` softFail) join candidate formation as GHOSTS — they keep the slot contested, can neither re-promote (leads never resurrect) nor demote further, and a user-discarded ghost stops blocking (a human's "not them" resolves the contest). New discriminating evidence still claims a contested slot (the escape hatch); a ghost twin row's stored discrimination transfers to its live registration twin; a ghost with the batch record's own id is its stale copy, filtered out. 6 ghost tests from the live store shape; full suite 3,468 all green.
