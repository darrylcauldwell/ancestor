...
## 6. iOS shell (Change 3) — IN PROGRESS (shell built 2026-07-09)

> **Built and simulator-verified** (commit f356a8c): `Ancestor-Viewer-iOS` target hand-authored in the pbxproj (mirrors the TV target; bundle `dev.dreamfold.Ancestor-Viewer`, iOS 26, iPhone+iPad, CloudKit entitlements copied). TreeScreen: full pan (drag) / zoom (pinch, 0.35–2.5×) / tap-to-open-person via `CanvasTransform.toLayout` hit-testing; fit-to-width + vertical centring on focal change; "Focus tree here" in the person sheet re-roots the layout; toolbar refresh. ViewerModel shared with the TV shell (copied per the no-forced-abstraction rule). **Cross-shell fix found during this change:** both viewers originally drew screen-space rects with an unscaled GraphicsContext, so fixed theme fonts overflowed shrunken nodes — corrected to the faithful macOS `TreeGraphView` pattern (`context.translateBy` + `context.scaleBy`, draw in layout coordinates; fonts scale with zoom); TV theme re-proportioned to the 160pt node with the canvas scale (1.5) providing 10-ft enlargement. Simulator screenshots verified on iPhone 17 Pro + Apple TV 4K. **Remaining acceptance:** owner's real iPhone via `.private` scope against production (after first prod publish), iPad pass, real-tree scale check.
