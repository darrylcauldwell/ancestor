...
### Change 3 — Plumbing spike (timeboxed, decision gate)
1–2 day spike: model §4's five record types in SQLiteData; verify (a) FK-root/one-FK sharing constraints can express Person↔Relationship↔LifeEvent, (b) enforced read-only CKShare works for a second test account, (c) the presence-diff/tombstone semantics of §4.1 are expressible. **Decision recorded here as an amendment**: adopt SQLiteData for the *published store only*, or hand-roll CKSyncEngine (tokens, tombstones, retry-after, CKAsset lifecycle). The canonical GRDB store is untouched either way.
