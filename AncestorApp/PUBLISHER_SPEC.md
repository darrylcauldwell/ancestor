...
### Change 3 — Plumbing spike (decision gate) — CLOSED 2026-07-04
**Decision CONFIRMED by runtime proof against the live development environment: adopt SQLiteData 1.6.6 for the published store only.** All three spike criteria passed (`PublishSpikeTests.runtimeProof`, env-gated with RUN_PUBLISH_SPIKE=1): (a) SyncEngine accepted the five-table published shape; (b) all records server-acked by the dev environment in ~2s; (c) hierarchy CKShare created from the manifest row. Two runtime findings binding on Change 4: **PublishEngine must call `SyncEngine.sendChanges()` explicitly after writing the store** — CKSyncEngine's scheduler defers sends indefinitely in short-lived/idle processes (91s of silence vs 2s with the explicit pump), and an on-demand publisher is exactly the "Backup now" case the API documents; and **share record names follow `share-<uuid>:<tableName>`**, not the `cloudkit.share` prefix.
...
### Change 4 — PublishEngine + CloudKit stand-up — SHIPPED 2026-07-04 (af761fe, 33a35b2, 700e6fc)
Delivered: `PublishedStore` (five `@Table` types; checksum-diffed apply; rowid-stable update-in-place proven by test; presence deletes; asset blobs hashed so unchanged photos never re-upload), `PublishEngine` (accountStatus pre-flight → generation guard (server ≤ local+1 allows own interrupted attempt) → store reconcile → per-project SyncEngine with explicit `sendChanges()` → ack wait → generation commit only after full server ack; every failure throws — no `try?`), `PublishInputs` shared assembly (bundle exporter re-pointed — the two surfaces cannot drift), pre-publish review screen (per-person resolved policy + override pickers; explicit-acknowledgement gate blocks publish until every person confirmed; deliberate override counts as acknowledgement), PRIVACY.md + store descriptions. Live E2E (env-gated `RUN_PUBLISH_E2E=1`): publish gen 1 (4/4 acked) → single-field delta gen 2 (2 updates, 2 untouched) → idle gen 3 (manifest-only traffic). Remaining acceptance items live with the user: first real-project publish through the review screen; console eyeball. `published_state` (v30) confirmed unused under SQLiteData — retained, dormant.

#### Original Change 4 text (for reference)
...
> **Amendment (2026-07-10) — media opt-in UI shipped.** The review sheet gained People/Media tabs: the Media tab lists every profile-targeted photo/document (transcriptions excluded per §4.2) with a per-attachment share toggle writing `publish_media`, the owner named on each row, an inert-opt-in warning when the owner doesn't resolve `full` (§5 — the projection drops it), and a footer stating the shared count and that assets bill the owner's iCloud quota. This closes the §7 media-opt-in risk mitigation's UI gap — before this, `publish_media` was always empty and no publish carried media.

> **Amendment (2026-07-08) — first-real-publish post-mortem and the shared-zone model.** The first publish of the real tree (967 records) surfaced three defects the fixture E2E had missed, plus an architecture correction:
> 1. **Write-before-engine**: store rows were written before the SyncEngine attached, but sqlite-data's change triggers exist only while an engine is attached — deltas were invisible to sync. Fixed: `engine.start()` is awaited BEFORE `apply()`.
> 2. **The wipe**: a publish into a zone whose name was recently deleted (his Unpublish → republish) gets `zone deleted` answers mid-upload, and the engine purges local rows — 967 rows lost mid-publish while 4-row fixtures won the race. Reproduced in miniature by test hygiene recycling a zone name.
> 3. **False success**: completion accounting used an unscoped ack count and accepted `0 ≥ 0` — generation 1 was committed with an empty store. Fixed: acks scoped to the five record types, pending-queue drain required, and an empty store now throws `storeInconsistent`.
> **Architecture correction**: CKSyncEngine syncs the whole database scope, so per-project zones under per-project stores cross-contaminate (project B fetches project A's records, then B's presence-deletes tombstone A). §2/§4 are amended: ONE shared published store (`published.sqlite`), the library's default zone, per-project isolation by manifest lineage (scoped presence-deletes), and **Unpublish = tombstoning the project's rows** (CloudKit deletes the CKShare with its root record) — the app never deletes zones. Zone-per-project was already obsolete for sharing (manifest-rooted hierarchy since Change 3). All re-proven live: the E2E now server-verifies the delta (fetch-after-publish), the erasure (record gone), and the republish-after-unpublish (identical UUIDs back on the server — sqlite-data #418 did not bite tombstone-then-reinsert).

> **Correction (2026-07-04, late):** the Change 3 spike and Change 4 E2E initially "passed against the development environment" while actually talking to **sqlite-data's built-in mock** — the library substitutes `MockCloudContainer`/`MockSyncEngine` whenever swift-dependencies detects a test context (`guard context == .live`). Both suites now force `prepareDependencies { $0.context = .live }` and have been **re-proven genuinely live**: spike 3/3 in 8.2s with a real icloud.com share URL; E2E 6/6 in 18.3s with the zone, records, and record types independently confirmed via `cktool export-schema`. Every env-gated CloudKit test MUST force live context; the tell for a mocked run is an unchanged dev-environment schema.

### Change 5 — Share flow + unpublish — SHIPPED 2026-07-04
Delivered: `PublishSharing` (idempotent fetch-or-create share rooted at the manifest — SQLiteData reuses the existing share from sync metadata; engine warm-up + bounded unknownItem retry for share-right-after-publish); Apple's native macOS cloud-sharing window (`NSSharingService(.cloudSharing)` + `NSItemProvider.registerCloudKitShare` — no hand-rolled participant UI needed; invite AND manage in one surface); "Family Sharing…" + "Unpublish Tree…" in the project context menu with destructive confirmation; "Invite Family…" on the post-publish success screen; unpublish = server-side zone deletion (participants evicted, share dies, records erased — the GDPR path) + full local store/metadatabase cleanup, idempotent. E2E proves: share idempotency, unpublish idempotency, and republish-after-unpublish continuing the generation sequence with byte-identical record UUIDs (§4.1 permanence, live).
**Deferred to Phase 4 (viewer):** participant-acceptance validation (accepting a share requires a client app on the participant device — the first viewer TestFlight to the second-account iPhone covers "fetches read-only / write attempts fail server-side / revocation removes access").

#### Original Change 5 text (for reference)
...
### Change 6 — Publish-time bio synthesis — SHIPPED 2026-07-05
Delivered: `PublishBioBuilder` — the LifeEvent→NarrativeLifeEvent adapter over the untouched shared `templateNarrative`, building prose from COMMITTED facts only (profile vitals, spouse edges, census life_events). Relative redaction at the adapter seam: nameOnly spouse = name without marriage year/place (facts about both parties strip); omit spouse = no sentence; omitted children absent, nameOnly children bare name, full children carry (b. year); sensitive events never reach prose. Children sentence composed OUTSIDE the shared template (research-path prose unchanged). Neutral citation uses fixed epoch date — bundle byte-determinism preserved. Snapshot test pins exact prose; the spec's canonical scenario (deceased subject + living spouse + omitted child) is tested verbatim. Projection fills bioText for `full` persons only; §5 zero-leakage holds for redacted persons.

#### Original Change 6 text (for reference)
