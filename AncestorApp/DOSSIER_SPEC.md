...
1. **No API/cloud model tier** — local MLX only (in-app Claude tier removed pre-App-Store). The provider seam is nonetheless deliberate: everything model-facing routes through `DossierInterpreter`'s selection/smoothing contracts, so a future BYO-key frontier tier (decision-gated: privacy consent + living-people redaction on outbound prompts) would slot behind the same cage without design change — better selections and prose, same invariants. Explored 2026-07-13; user's current position: dev-side frontier use (eval judging, build assistance) is the higher-value lane, so the user-facing tier stays parked as a future decision, not a plan.
