...
**Status: IN BUILD — Change 1 SHIPPED 2026-07-31; Changes 2–6 next.** Commits reference `#T9-Change1`…`#T9-Change6`. (Drafted 2026-07-13 from a three-design competition; small-model-first won 9/9/9.)

**Change 1 as-built (2026-07-31):** `GroundedProse.swift` (AncestorKit — `ProvenanceRef`/`GroundedSentence` failable-init grounding, `ConfidenceVocabulary` + banned lexicon, `GroundedProseVerifier` core checks incl. digit protection, `Dossier`/`DossierSection`/`DossierFooter` + skeleton SHA-256); `DossierAssembler` D0–D5+D7 (pure — plain-value inputs, no DB handle, so zero-writes holds by construction); `negativeSearches(profileID:)` + `NegativeSearchRow` (isCleanNegative = zero/NULL); `AppState.assembleDossier(for:)`; `DossierEntryRow`/`DossierView` profile-page door (`.sheet(item:)`); MCP `ancestor://profile/{id}/dossier`. 10 assembler acceptance tests + 2 MCP resource tests; suites 3,496 / 141 green. **Deliberate deviations:** (1) the MCP resource renders the skeleton from the same ROWS via its own queries (data-level parity) — FieldResearcherMCP deliberately has no AncestorKit dependency, so shared-code parity would couple the standalone binary to the whole domain layer; (2) `ProvenanceRef` gained two additive cases the profile-page door needs — `.researchRun(id)` (D0 stored-run standing when no live `GPSScore` is in memory) and `.profileField(profileID:field:)` (structural-gap anchors); (3) live GPS-criterion lines and D5 cluster comparison render only when a run result is in memory (review surfaces — Change 6's refresh lifecycle wires them); the profile door shows the stored gps_score summary honestly; (4) review-surface doors (§4.3 b/c) ride with Change 3's steering; (5) **schema note:** v42 is TAKEN (negative-search outcome columns, 2026-07-28) — Change 2's migration is the next free number (v57+), not `v42_dossier`.
