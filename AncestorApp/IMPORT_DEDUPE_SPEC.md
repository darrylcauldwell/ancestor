...
**Status: Changes 1–6 SHIPPED. 1–3** (`OrphanStubDetector` / `OrphanStubRule`, import-time cleanse, `ProfileMergeEngine`); **4–6 shipped 2026-07-18** — the phantom-spouse extension (`PhantomSpouseDetector` / `PhantomSpouseRule`, guided cleanse card + v49 reviewed marker, post-import + on-demand scan wiring), plus **option-A citation salvage** on the merge path. Manual-merge oracle verified first (William Henry Keyworth → exactly Emma Gladwin + Elizabeth Wallace, no duplicate edge). Commits reference `#ID-Change4`…`#ID-Change6`.

## Build notes (2026-07-18)

- **Name-containment routes on shared name *tokens*, not diminutives** (Change 4). `nameContained` fires when the phantom's given name is a token of the target's full name — so "Elizabeth" Carroll routes to the "Elizabeth" wife (Wallace), while "Gerty" (a diminutive of "Gertrude", not a token) presents the documented-spouse *choices*. The spec's original AC1 ("nameContainment true for Gerty, false for Carroll") assumed diminutive-matching **and** that Wallace's profile stored "Gertrude"; neither holds on real data, so the shipped behaviour is the reverse and *more useful* — both phantoms still reach Wallace (Carroll auto-suggested; Gerty via the picker → the verified end-state). Diminutive matching (Gerty→Gertrude) would need a nickname table and is a deliberate non-goal.
- **Option A — citation salvage** (`ProjectDatabase.salvageCitedFieldSources`, wired into the single `AppState.performProfileMerge` choke point used by the card *and* `CompareProfilesView`). Before the hard-delete, every loser `field_sources` row carrying a real `citation_json` is re-pointed to the winner; bare GEDCOM name provenance is left to be deleted (re-applying "Gerty" as a name value on the winner would be wrong). Closes the general-merge evidence-loss gap surfaced when Gerty/Carroll were merged into Wallace. Empirically those phantoms had no cited records (research re-derives citations), so this is a safety net — but it now protects every merge.
- **AC1-vs-reality resolution is intentional.** Per "Swift is the product, improve it" — the shipped behaviour has tests (`PhantomSpouseDetectorTests`, 10) asserting the real routing, not the idealised AC.
