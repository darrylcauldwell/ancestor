...
**Status: COMPLETE — all changes shipped. This spec is now git-only history; nothing here is queued.**
- **Change 5 (muddle-detector deep-links)** — SHIPPED 2026-07-21 (`60f8fa8`). `MuddledIdentityRule` + kin `RecordAfterDeathRule` findings gain a "Review records" Tasks action that opens the flagged profile's Full Detail with the Sources & Records ledger expanded + scrolled into view (new `AppState.requestLedgerReviewProfileID` intent consumed by `ProfileDetailView` via a `ScrollViewReader`, on appear/switch/intent-change). Rule IDs pinned by `ledgerReviewDeepLinkRuleIDsExist`.

Prior status (retained for the design record):
**Changes 1+2+3+4 SHIPPED. Change 5 (muddle deep-links) remains.**
