...
final class AppState {
    ...
    /// alert with a one-click "Open Settings" instead of a dead-end OK. Shared
    /// by the Family Tree write-back (upload) flow — the only FamilySearch
    /// capability that ingests/emits FamilySearch data now that records and
    /// record-hint ingestion have been dropped (owner 2026-08-07: FamilySearch
    /// is a tree read/write integration, not a records source).
