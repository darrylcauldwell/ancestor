...
final class ResearchPipeline {
    ...
    private func fetchSpouseHeldMarriageRecords(
        ...
        for t in targets {
            ...
            // The target's reference tuple is authoritative: it is the same
            // register page, and the page-lookup echoes back BLANK vol/page
            // (queried by them), so `e.volume`/`e.page` are empty strings —
            // `??` won't fall back on "", which silently voided the key.
            // Force year/vol/page/quarter/district from the target; keep the
            // pulled entry's own name/common/spouseName.
            ...
            for e in entries where ScoringRules.nameSimilarity(
                ...
                out.append(.marriage(MarriageRecord(
                    ...
                    quarter: nonEmpty(t.quarter) ?? nonEmpty(e.quarter),
                    district: nonEmpty(t.district) ?? nonEmpty(e.district),
                    volume: t.volume,
                    page: t.page,
